...
The Ask box routes a few keywords (payload, tyres, power, water / gas / tanks / cassette). Toilet, loo and porta potty go to Cassette. Common electrical appliance words (microwave, fridge, kettle, heater, 12V, watts…) open Power — daily power by default, or Battery / Solar / Inverter / Wire when the query names that tool. Tyre pressure / psi / bar stay on Tyres. If nothing matches, the question (and optional email) is passed to the site owner via `POST /api/ask`. It does **not** invent tyre pressures, weights or legal advice. Replies are not automated.
